...
public abstract class IntrospectedTable {
    ...
    protected enum InternalAttribute {
        ...
        ATTR_BUSINESS_RECORD_TYPE,
        ATTR_DATA_SERVICE_TYPE,
        ATTR_DATA_SERVICE_IMPL_TYPE,
        ...
        ATTR_LIST_BY_CONDITION,
        ATTR_UPDATE_BY_CONDITION,
        ATTR_QUERY_BY_CONDITION
    }
    ...
    public void validateCreateUpdateTime() {
        String createTimeColumn = this.tableConfiguration.getCreateTimeColumn();
        String updateTimeColumn = this.tableConfiguration.getUpdateTimeColumn();
        IntrospectedColumn column = getColumn(createTimeColumn);
        if (column == null) {
            System.err.println("[配置错误]: 找不到对应的生成时间字段，请检查配置");
        }
        column = getColumn(updateTimeColumn);
        if (column == null) {
            System.err.println("[配置错误]: 找不到对应的更新时间字段，请检查配置");
        }
    }

    public boolean isCreateTimeColumn(IntrospectedColumn introspectedColumn) {
        return this.tableConfiguration.getCreateTimeColumn().equals(introspectedColumn.actualColumnName);
    }

    public boolean isUpdateTimeColumn(IntrospectedColumn introspectedColumn) {
        return this.tableConfiguration.getUpdateTimeColumn().equals(introspectedColumn.actualColumnName);
    }
    ...
    public List<IntrospectedColumn> getPrimaryKeyColumns(CandidateKey candidateKey) {
        if (candidateKey == null) {
            return getPrimaryKeyColumns();
        }
        List<String> columnNames = candidateKey.getColumnNames();
        List<IntrospectedColumn> candidateColumns = new ArrayList<>();
        for (String columnName : columnNames) {
            IntrospectedColumn column = getColumn(columnName);
            if (column != null) {
    ...
    public List<IntrospectedColumn> getNonPrimaryKeyColumns(CandidateKey candidateKey) {
        if (candidateKey == null) {
            return getNonPrimaryKeyColumns();
        }
        List<String> columnNames = candidateKey.getColumnNames();
        List<IntrospectedColumn> nonCandidateColumns = getAllColumns();
        for (String columnName : columnNames) {
            IntrospectedColumn column = getColumn(columnName);
            if (column != null) {
                nonCandidateColumns.remove(column);
            }
        }
        return nonCandidateColumns;
    }
    ...
    public String getBusinessRecordType() {
        return internalAttributes.get(InternalAttribute.ATTR_BUSINESS_RECORD_TYPE);
    }

    public String getDataServiceType() {
        return internalAttributes.get(InternalAttribute.ATTR_DATA_SERVICE_TYPE);
    }

    public String getDataServiceImplType() {
        return internalAttributes.get(InternalAttribute.ATTR_DATA_SERVICE_IMPL_TYPE);
    }
     ...
     */
    protected String getIbatis2SqlMapFileName() {
    ...
    private String getMyBatis3FallbackSqlMapNamespace() {
     ...
     */
    protected String getIbatis2SqlMapPackage() {
     ...
     *
     */
    private void calculateXmlAttributes() {
        ...
        setUpdateByCondition("updateByCondition");
    ...
    public String getUpdateByConditionStatementId() {
        return internalAttributes.get(InternalAttribute.ATTR_UPDATE_BY_CONDITION);
    }
    ...
    public String getQueryByConditonStatementId() {
        return internalAttributes.get(InternalAttribute.ATTR_QUERY_BY_CONDITION);
    }

    public String getListByConditionStatementId() {
        return internalAttributes.get(InternalAttribute.ATTR_LIST_BY_CONDITION);
    }
    ...
    private String calculateJavaClientInterfacePackage() {
        JavaClientGeneratorConfiguration config = context.getJavaClientGeneratorConfiguration();
        if (config == null) {
            return null;
        }
        return config.getTargetPackage() + fullyQualifiedTable.getSubPackage(isSubPackagesEnabled(config));
    }

    private void calculateJavaClientAttributes() {
        ...
        sb.append('.');
        sb.append(fullyQualifiedTable.getDomainObjectName().substring(1));
        sb.append("DAOImpl"); 
    ...
    private String calculateBusinessModelPackage() {
        BusinessModelGeneratorConfiguration config = context.getBusinessModelGeneratorConfiguration();
        return config.getTargetPackage() + fullyQualifiedTable.getSubPackage(isSubPackagesEnabled(config));
    }

    private void calculateModelAttributes() {
        ...
        if (context.getBusinessModelGeneratorConfiguration() != null) {
            String businessPackage = calculateBusinessModelPackage();
            sb.setLength(0);
            sb.append(businessPackage);
            sb.append('.');
            sb.append(fullyQualifiedTable.getDomainObjectName().substring(1));
            sb.append("BO");
            setBusinessRecordType(sb.toString());
            ...
            if (context.getBusinessModelGeneratorConfiguration().isGenerateDataService()) {
                ...
                sb.append(".impl.");
                sb.append(fullyQualifiedTable.getDomainObjectName().substring(1));
                sb.append("DataServiceImpl");
                setDataServiceImplType(sb.toString());
            }
        }
    ...
    private String calculateIbatis2SqlMapFileName() {
        return fullyQualifiedTable.getIbatis2SqlMapNamespace().substring(1) + "_SqlMap.xml";
    }
    ...
    private String calculateMyBatis3XmlMapperFileName() {
        return fullyQualifiedTable.getDomainObjectName().substring(1) + "Dao.xml";
    }

    private String calculateIbatis2SqlMapNamespace() {
     ...
     * mybatis SQL里面的命名空间
     */
    private String calculateMyBatis3FallbackSqlMapNamespace() {
        return calculateSqlMapPackage() + '.' + fullyQualifiedTable.getDomainObjectName().substring(1) + "Mapper";
    }

    private String calculateSqlMapFullyQualifiedRuntimeTableName() {
        return fullyQualifiedTable.getFullyQualifiedTableNameAtRuntime();
    }

    private String calculateSqlMapAliasedFullyQualifiedRuntimeTableName() {
    ...
    private void setDAOImplementationType(String DAOImplementationType) {
        internalAttributes.put(InternalAttribute.ATTR_DAO_IMPLEMENTATION_TYPE, DAOImplementationType);
    }

    private void setDAOInterfaceType(String DAOInterfaceType) {
        internalAttributes.put(InternalAttribute.ATTR_DAO_INTERFACE_TYPE, DAOInterfaceType);
    }

    private void setPrimaryKeyType(String primaryKeyType) {
        internalAttributes.put(InternalAttribute.ATTR_PRIMARY_KEY_TYPE, primaryKeyType);
    }

    private void setBaseRecordType(String baseRecordType) {
        internalAttributes.put(InternalAttribute.ATTR_BASE_RECORD_TYPE, baseRecordType);
    }

    private void setBusinessRecordType(String businessRecordType) {
        internalAttributes.put(InternalAttribute.ATTR_BUSINESS_RECORD_TYPE, businessRecordType);
    }

    private void setDataServiceType(String dataServiceType) {
        internalAttributes.put(InternalAttribute.ATTR_DATA_SERVICE_TYPE, dataServiceType);
    }

    private void setDataServiceImplType(String dataServiceImplType) {
        internalAttributes.put(InternalAttribute.ATTR_DATA_SERVICE_IMPL_TYPE, dataServiceImplType);
    }

    private void setRecordWithBLOBsType(String recordWithBLOBsType) {
    ...
    private void setMyBatis3JavaMapperType(String mybatis3JavaMapperType) {
    ...
    private void setMyBatis3SqlProviderType(String mybatis3SqlProviderType) {
